 ...
 *
 * $Id: patForms.php,v 1.2 2006/06/21 02:56:09 eric Exp $
 *
 * @package		patForms
 ...
 */
define('PATFORMS_WARNING_LOCALEFILE_NOT_FOUND', 1050);
...
class patForms
{
   ...
   /**
	* flag to indicate, whether autovalidation has been
	* already called
	*
	* @access	private
	* @var		boolean
	*/
	var	$autoValidateUsed = false;
	...
	* @see		registerEvent()
	*/
	var	$_validEvents = array('onInit', 'onValidate', 'onSubmit', 'onError', 'onSuccess', 'onRender');
   ...
   /**
    * Sets the default-encoding of the form / the site the form is displayed on.
    * 
    * This affects the encoding/decoding of html-entities in the form's element-value 
    * and attributes.
    * 
    * @static 
    * @access   public
    * @param    string  $encoding   the ISO code for the desired encoding (UTF-8, ISO-8859-1, ...)
    */
	function setDefaultEncoding( $encoding )
	{
		patForms::setStaticProperty( 'defaultEncoding', $encoding );
	}
	...
	*/
	function setLocale( $locale, $languageFile = null )
	{
		if (!is_null($languageFile)) {
			...
			if (patErrorManager::isError($languageData)) {
				return $languageData;
			}
	...
	*/
	function parseLocaleFile($filename)
	{
	    if (!file_exists($filename) ||
	    	...
	    	return patErrorManager::raiseWarning(PATFORMS_WARNING_LOCALEFILE_NOT_FOUND, 'Could not locale-file ' . $filename . '.');
	    }
	...
	*/
	function setSubmitted( $state )
	{
		if ($state == true) {
			...
			if (patErrorManager::isError($eventState)) {
				return $eventState;
			}
			if ($eventState === false) {
				return false;
	...
	*
	* @access	public
	* @param	string|object patForms_Filter     The filter object to apply or the name of the filter
	* @param    array                             The parameters for the filter, if only the name has been passed.
	* @return   boolean                           true, if the filter could be applied
	* @see		patForms_Element::applyFilter()
	*/
	function applyFilter(&$filter, $params = null)
	{
		if (empty($this->elements)) {
			return true;
		}
		...
		if (is_string($filter)) {
			$filter = &$this->createFilter($filter, $params);
		...
		$cnt = count($this->elements);
		for ($i = 0; $i < $cnt; $i++) {
		    $result = $this->elements[$i]->applyFilter($filter);
   ...
   /**
    * Wrapper method that adds an attribute filter to all 
    * elements of the form at once instead of having to do 
    * it for each element.
    *
    * @access   public
    * @param    string|object patForms_AttributeFilter The filter object to apply or the name of the filter
    * @param    array  The parameters for the filter, if only the name has been passed.
    * @return   boolean  true, if the filter could be applied
    * @see      patForms_Element::applyAttributeFilter()
    */
    function applyAttributeFilter(&$filter, $params = null)
    {
        if (empty($this->elements)) {
            return true;
        }
        ...
        if (is_string($filter)) {
            $filter = &$this->createAttributeFilter($filter, $params);
            if (patErrorManager::isError($filter)) {
                return $filter;
            }
        }
        ...
        $cnt = count($this->elements);
        for ($i = 0; $i < $cnt; $i++) {
            $result = $this->elements[$i]->applyAttributeFilter($filter);
            if (patErrorManager::isError($result)) {
                return $result;
            }
        }
	...
	* @return	object patForms             The new patForms object.
	*/
   ...
   /**
	* Retrieves a reference to the form's current renderer object
	* 
	* @access	public
	* @return	null|object
	*/
	...
	*/
	function renderForm( $args = null )
	{
		...
		if (!$this->isSubmitted() || $this->autoValidate !== true) {
			$result = $this->triggerEvent('Init');
			if (patErrorManager::isError($result)) {
				return $result;
			}
			$result = $this->triggerEvent('Render');
			if (patErrorManager::isError($result)) {
				return $result;
			}
	...
	*/
	function validateForm( $revalidate = false )
	{
		...
		/**
		 * let listeners validate the form
		 */
		if ($valid === true) {
	...
	* Apply rules.
	*
	* This calls the validation method of all rules
	* that have been added to the form.
	*
	* If any of the rules is not valid, then this
	...
	*/
	...
	{
	    $module = &patForms::_createModule('Renderer', $name);
		return $module;
	}
	...
	*/
	...
	{	    
		$module = &patForms::_createModule('Storage', $name);
		return $module;
	}
	...
	*
	* @access	public
	* @param	string						The name of the datasource to create - have a look at the Datasource/ subfolder for a list of available storage containers.
	* @return	object patForms_Storage		The storage container, or error object
	*/
	function &createDatasource($name)
	{
		$module = patForms::_createModule('Datasource', $name, false);
		return $module;		
	}
   ...
   /**
    * Creates a new attribute filter and returns it.
    *
    * You may pass an array as second parameter that contains
    * parameters for the filter. patForms will check for setter methods
    * for all keys and set the corresponding values.
    ...
    *
    * @access   public
    * @param    string The name of the filter to create - have a look at the AttributeFilter/ subfolder for a list of available filters.
    * @param    array  Optional parameters for the filter, if you provide a parameter, make sure the filter implements a set[Paramname]() method.
    *                  This will be automated with interceptors in the PHP5 version of patForms
    * @return   object patForms_AttributeFilter The filter, or error object
    */
    function &createAttributeFilter($name, $params = null)
    {
        $filter = &patForms::_createModule( 'AttributeFilter', $name );
	...
	*/
	function &_createModule($type, $name, $hasBaseClass = true)
	{
		if ($hasBaseClass) {
	...
	*/
    function convertElement($name, $type)
    {
		$element = &$this->getElement($name);
		$element = patForms_Element::convertElement($element, $type);
	}
	...
	* @todo     add a second parameter to define how errors of elements within container elements (Group) should be handled
	*           should match the getValues() behaviour
	*/
	...
	*/
	function getValues( $fields = null, $type = PATFORMS_VALUES_NESTED )
	{
		...
		for ($i = 0; $i < $cnt; ++$i) {
			...
			if (!is_array($tmpVal) || $this->elements[$i]->isContainerElement()) {
				$values[$name] = $tmpVal;
				continue;
			}
	...
	*/
	function serializeStartHtmlDefault()
	{
		...
		$content = patForms::createTag( 'form', 'opening', $attributes );
	...
	*/
	function serializeStartHtmlReadonly()
	{
		...
		return patForms::createTag( 'form', 'opening', $attributes );
	}
	...
	*/
	...
	{
		return	patForms::createTag( 'form', 'closing' );
	}
	...
	function createTag( $tagname, $type = "full", $attributes = array(), $value = false )
	{
		switch( $type )
		{
			...
			case "full":
				if( $value === false )
				{
					return patForms::createTag( $tagname, "empty", $attributes );
				}

				return patForms::createTag( $tagname, "opening", $attributes ).htmlentities( $value ).patForms::createTag( $tagname, "closing" );
				break;
		}
	}
	...
	*/
	...
	{
		if(!isset($this->options[$option][$parameter])) {
			return false;
		}
	...
	*/
	function toXML( $namespace = null )
	{
		...
		return	patForms::createTag( $tagName, "full", $attributes, $elements );
	}
	...
	*/
	function getScripts()
	{
		// let elements and rules recursively register their scripts to the form
		foreach ($this->elements as $element) {
			$element->registerJavascripts($this);
		}
		foreach ($this->_rules as $rule) {
			$rule['rule']->registerJavascripts($this);
		}
		...
		if ($optionParams['jsInclude'] === true) {
			$script = "\n";
			foreach ($this->globalJavascript as $scriptFile) {
			...
			$script .= implode ('', $this->instanceJavascript);
			$script .= '</script>';
		} else {
   ...
   /**
	* Registers a global script to the form
	*
	* This method gets called from elements and rules.
	*
	* @access	public
	...
	*/
	...
	{
		$this->globalJavascript[$type] = $script;
	}

   /**
	* Registers an instance script to the form
	*
	* This method gets called from elements and rules.
	*
	* @access	public
	* @param	string $script a script (source, not a filename)
	* @see		patForms::getScripts()
	* @see		patForms_Element::registerJavascripts()
	* @see		patForms_Rule::registerJavascripts()
	*/
	...
	{
		$this->instanceJavascript[] = $script;
	}
